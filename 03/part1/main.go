...
// super dumb
func getOverlap(path1, path2 [][2]int) [][2]int {
	var overlap [][2]int
	for _, path1Seg := range path1 {
		for _, path2Seg := range path2 {
			if path1Seg == path2Seg {
				overlap = append(overlap, path1Seg)
			}
...
func main() {
	...
	pathsOverlap := getOverlap(path1, path2)
	var manhatinzed []int

	for _, overlap := range pathsOverlap {
		m := manhatanize(overlap)
		if m != 0 {
			manhatinzed = append(manhatinzed, manhatanize(overlap))
		}
	}

	fmt.Print(min(manhatinzed...))
}
