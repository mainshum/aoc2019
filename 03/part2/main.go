...
func interpolateTwoPoints(startPointX, startPointY, endPointX, endPointY int) [][2]int {
	...
	if startPointX != endPointX {
		...
		if endPointY > startPointY {
			for k := startPointY + 1; k <= endPointY; k++ {
				retval = append(retval, [2]int{startPointX, k})
			}
		} else {
			for k := startPointY - 1; k >= endPointY; k-- {
				retval = append(retval, [2]int{startPointX, k})
...
func getPath(input []string) [][2]int {
	var path [][2]int
	path = append(path, [2]int{0, 0})
	for _, val := range input {
		lastX, lastY := path[len(path)-1][0], path[len(path)-1][1]
		dir := string(val[0])
		pathLen, _ := strconv.Atoi(val[1:len(val)])
		switch dir {
		case "R":
			path = append(path, interpolateTwoPoints(lastX, lastY, lastX+pathLen, lastY)...)
		case "L":
			path = append(path, interpolateTwoPoints(lastX, lastY, lastX-pathLen, lastY)...)
		case "U":
			path = append(path, interpolateTwoPoints(lastX, lastY, lastX, lastY+pathLen)...)
		case "D":
			path = append(path, interpolateTwoPoints(lastX, lastY, lastX, lastY-pathLen)...)
		}
	}
...
func manhatanize(input [2]int) int {
	return abs(input[0]) + abs(input[1])
}

// super dumb
func getOverlapDistanceFrom0(path1, path2 [][2]int) []int {
	var overlapDistances []int
	for path1Ind, path1Seg := range path1 {
		if path1Seg == [2]int{0, 0} {
			continue
		}
		for path2Ind, path2Seg := range path2 {
			if path1Seg == path2Seg {
				overlapDistances = append(overlapDistances, path1Ind+path2Ind)
			}
...
func main() {
	path1 := getPath(strings.Split(wire1, ","))
	path2 := getPath(strings.Split(wire2, ","))

	pathsOverlap := getOverlapDistanceFrom0(path1, path2)

	// get index of each overlap in both paths

	fmt.Print(min(pathsOverlap...))
}
